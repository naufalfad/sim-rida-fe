...
export default function EditRecommendationPage() {
  ...
  // Redirect if locked
  useEffect(() => {
    if (rec && (rec.status === 'APPROVED' || rec.status === 'PUBLISHED' || rec.status === 'UNDER_REVIEW')) {
      toast('Rekomendasi terkunci atau sedang ditinjau (Read-Only).', 'warning');
  ...
  const [expectedPolicyImpact, setExpectedPolicyImpact] = useState('');
  ...
  useEffect(() => {
    if (rec) {
      ...
      setRecommendationDescription(rec.recommendationDescription || '');
      setExpectedPolicyImpact(rec.expectedPolicyImpact || '');
      setPriority(rec.priority || 'MEDIUM');
      setPrimaryRecipientId(rec.primaryRecipientId || '');
      setSupportingRecipientIds(rec.supportingRecipientIds || []);
      setSelectedFindingIds(rec.findingIds || []);

      setBasisReport(rec.researchBasis.includes('Research Report'));
      setBasisBrief(rec.researchBasis.includes('Policy Brief'));
      setBasisFindings(rec.researchBasis.includes('Findings'));
      setBasisConclusion(rec.researchBasis.includes('Conclusion'));
    }
  }, [rec]);
  ...
  const handleToggleSupportingOpd = (opdId: string) => {
    if (opdId === primaryRecipientId) return;
  ...
  const handleSave = () => {
    ...
    toast('Draf perbaikan rekomendasi berhasil disimpan.', 'success');
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <PageHeader
        ...
        description={`Sunting draf perbaikan rekomendasi BRIDA: "${rec?.title}"`}
      />
            ...
            <CardContent className="p-6 space-y-4 text-xs font-semibold">
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  rows={4}
                  className="block w-full px-3 py-2 border rounded focus:outline-none resize-none bg-white text-gray-900 focus:border-indigo-655 leading-relaxed font-semibold"
                />
              </div>
              ...
              {/* Priority Select */}
              <div className="space-y-1.5">
                <label className="block text-2xs font-bold text-gray-700 uppercase">Priority *</label>
                <select
                  value={priority}
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardContent className="p-6 space-y-4 text-xs font-semibold select-none">
              ...
              {findings.length === 0 ? (
                <span className="text-[10px] text-gray-455 italic">Tidak ada temuan terdaftar di Laporan Hasil.</span>
              ) : (
                  ...
                  {findings.map(f => {
                    ...
                    return (
                      <label key={f.id} className="flex items-start gap-2 py-1.5 cursor-pointer text-3xs hover:bg-slate-100/50 px-1 rounded">
                        ...
                        />
                        <div>
                          <span className="font-bold text-gray-850 block leading-tight">{f.title}</span>

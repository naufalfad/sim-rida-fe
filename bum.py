#!/usr/bin/env python3
# bum.py — Best Practice Codebase Aggregator (React / Vite / TypeScript / Tailwind / PWA)
import os
import sys
import re
import argparse
...
# Menyimpan seluruh konfigurasi penapisan secara terpusat untuk React/Vite PWA
# =========================================================================
class AggregatorConfig:
    ...
    # 3. Whitelist Folder yang Diizinkan untuk Ditelusuri (Frontend React / Vite PWA)
    ALLOWED_DIRS = {
        "src", "public", "components", "views", "modules", "services", "assets",
        "hooks", "context", "store", "utils", "types", "styles", "layouts"
    }

    # 4. Ekstensi Berkas Teks yang Diizinkan untuk Frontend React / Vite / TypeScript / CSS
    INCLUDE_EXTENSIONS = {
        ".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs", ".json", ".css", ".scss",
        ".html", ".svg", ".yaml", ".yml", ".md", ".env.example"
    }
    ...
    ESSENTIAL_ROOT_FILES = {
        ...
        "postcss.config.js", "index.html", ".env.example", "README.md"
    }
...
class CodebaseAggregator:
    ...
    def execute(self):
        ...
        if self.strip_comments:
            print("✂️ Pembersihan komentar & JSDoc diaktifkan untuk berkas TS/TSX/JS/CSS.")
        if self.exclude_tests:
        ...
        try:
            with open(self.output_file, "w", encoding="utf-8") as out_file:
                out_file.write("=== BRIDA SMART ANALYSIS (FRONTEND CODEBASE BUNDLE) ===\n\n")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Optimasi Bundel Kode Frontend React / Vite / TypeScript PWA")
